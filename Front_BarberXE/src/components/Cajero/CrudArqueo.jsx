...
function ArqueoDeCaja() {
    ...
    useEffect(() => {
        const inicializarDatos = async () => {
            try {
                ...
                // Obtener el empleado actual desde el token
                const user = LoginService.getCurrentUser();
                if (!user || !user.idEmpleado) {
                    throw new Error("No se pudo obtener la información del empleado actual");
                }
                console.log("Datos de autenticación:", localStorage.getItem('authData'));
                ...
                if (user.cargo.toLowerCase() !== 'cajero') {
                    setError("Solo los cajeros pueden realizar arqueos de caja");
                    setLoading(false);
                    return;
                }
                ...
                if (arqueoGuardadoId) {
                    try {
                        ...
                        // Verificar que el arqueo pertenece al empleado actual
                        if (arqueoData && !arqueoData.fechaCierre && arqueoData.empleado?.idEmpleado === user.idEmpleado) {
    ...
    const abrirArqueo = async (e) => {
        ...
        try {
            ...
            if (!empleadoActual || !saldoBase) {
                throw new Error("No se pudo identificar al empleado o falta el saldo base");
            }

            const response = await createArqueo({
                empleadoId: Number(empleadoActual.idEmpleado),
            ...
            localStorage.setItem('currentEmpleadoIdForArqueo', empleadoActual.idEmpleado );
                        ...
                        {loading && arqueos.length === 0 ? (
                            ...
                            <div className="space-y-4 overflow-y-auto" style={{ maxHeight: "600px" }}>
                                {arqueos.map((arqueo) => (
                                    <div
                                        key={arqueo.idArqueo}
                                        className={`border rounded-lg p-4 hover:bg-gray-50 transition-colors cursor-pointer ${arqueoSeleccionado?.idArqueo === arqueo.idArqueo ? 'border-blue-500 bg-blue-50' : ''
                                            }`}
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <span className={`px-3 py-1 rounded-full text-sm ${!arqueo.fechaCierre ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"
                                                }`}>
                            ...
                            <form onSubmit={abrirArqueo} className="space-y-4">
                                <div>
                                    ...
                                    <div className="w-full p-2 border rounded-md bg-gray-100">
                                        {empleadoActual?.nombre || "Cargando..."}
                                    </div>
                                </div>
