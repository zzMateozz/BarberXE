...
const ValidationMessage = ({ message, isValid }) => {
  ...
  return <div className={`text-sm mt-1 ${isValid ? "text-green-600" : "text-red-600"}`}>{message}</div>
}
...
const TableEmployees = ({ isCollapsed }) => {
  ...
  const getInputBorderClass = (value, error) => {
    if (!value) return "border-zinc-300"
        ...
        <div className="bg-white rounded-lg shadow-sm border border-zinc-200 overflow-hidden">
          ...
          {employees.length > 0 && (
            <div className="py-3 px-4 bg-zinc-50 border-t border-zinc-200 flex items-center justify-between text-xs text-zinc-500">
              <div>
                Mostrando <span className="font-medium">{employees.length}</span> empleados
              </div>
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
